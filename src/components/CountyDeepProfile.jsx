...
const { FiAward, FiTrendingUp, FiUsers, FiDollarSign, FiTarget, FiPieChart, FiActivity } = FiIcons;

const CountyDeepProfile = ({ county, allCounties }) => {
  // FIX: Hooks must be called before any conditional return
  const rankings = useMemo(() => {
    // Handle case where county or allCounties might be missing inside the hook
  ...
  // Early return can now safely happen after hooks
  if (!county) return null;

  // Mock sector data specific to the county's size/profile
  const sectorData = {
      ...
      >
          ...
          <div className="relative z-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="bg-white/5 rounded-2xl p-4 border border-white/10">
                ...
                <p className="text-[9px] text-gray-400 mt-2 font-medium">Top {Math.round((rankings.wageRank/rankings.totalCounties)*100)}% for compensation levels in the UP.</p>
              </div>
              ...
              <div className="bg-white/5 rounded-2xl p-4 border border-white/10">
                ...
                <p className="text-[9px] text-gray-400 mt-2 font-medium">Significant regional employer with {county.employment.toLocaleString()} active roles.</p>
              </div>
              ...
              <div className="bg-white/5 rounded-2xl p-4 border border-white/10">
                ...
                <p className="text-[9px] text-gray-400 mt-2 font-medium">Relatively {county.density > 8 ? 'dense' : 'moderate'} organizational footprint.</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="p-8 grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <div>
            ...
            <div className="mt-8 p-4 bg-gray-50 rounded-xl border border-gray-100">
               <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-[10px] font-black uppercase tracking-widest text-gray-900">Economic Multiplier</span>
               </div>
               <p className="text-[11px] text-gray-500 leading-relaxed">
                 Nonprofit activities in {county.name} generate an estimated 1.45x indirect job multiplier, supporting local supply chains and municipal services.
               </p>
            </div>
          </div>
          ...
          <div className="flex flex-col">
            <h3 className="text-sm font-black text-gray-900 uppercase tracking-widest mb-6 flex items-center gap-2">
              <SafeIcon icon={FiPieChart} className="text-teal-600" />
              Organizational Mix
            </h3>
            ...
            <div className="grid grid-cols-2 gap-4 mt-4">
              <div className="p-3 bg-blue-50 rounded-xl border border-blue-100">
                <div className="text-[9px] font-black text-blue-800 uppercase mb-1">Avg Compensation</div>
                <div className="text-xl font-black text-blue-900">${county.baseWage.toLocaleString()}</div>
              </div>
              <div className="p-3 bg-teal-50 rounded-xl border border-teal-100">
                <div className="text-[9px] font-black text-teal-800 uppercase mb-1">Entity Growth</div>
                <div className="text-xl font-black text-teal-900">+4.2%</div>
              </div>
            </div>
          </div>
        </div>
